...
Sistema de **Gestão de Combustíveis** desenvolvido em **CodeIgniter 3** com **PHP 8**, **MariaDB** e **Bulma.io**.  
Uma aplicação moderna, leve e eficiente para o controle de abastecimentos, consumo e relatórios de desempenho.
...
```bash
git clone https://github.com/SEU_USUARIO/sgcombustivel.git
...
CREATE DATABASE sgcombustivel CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

Em seguida, importe o arquivo SQL com a estrutura inicial:
mysql -u root -p sgcombustivel < database/sgcombustivel.sql

4️⃣ Configure o servidor Apache

Adicione um virtual host (em /etc/apache2/sites-available/sgcombustivel.conf):

<VirtualHost *:80>
    ServerName sgcombustivel.local
    DocumentRoot /var/www/html/sgcombustivel

    <Directory /var/www/html/sgcombustivel>
        AllowOverride All
        Require all granted
    </Directory>
</VirtualHost>
...
sudo a2ensite sgcombustivel.conf
sudo systemctl reload apache2

5️⃣ Teste o sistema

Abra no navegador:
http://sgcombustivel.local

Se tudo estiver certo, o painel inicial do SGCombustível será exibido 🎉
